...
int8_t readTSL() {
  ...
  if (event.light == 0) {
    ...
    return -1;
  } else return event.light;
}

uint8_t angleToWall(uint8_t theta1, uint8_t theta2) {  //should return angle relative to east returns -90 to 90 deg
  uint32_t sum_x = 0;
  uint32_t sum_y = 0;
  uint32_t sum_x2 = 0;
  uint32_t sum_y2 = 0;
  uint32_t sum_xy = 0;
  uint8_t n = 0;
  for (uint8_t i = theta1; i < theta1 - theta2; i++) {
...
void calibrateToWall(uint8_t theta1, uint8_t theta2) {
  //will adjust robot position to be as parellel to wall as possible
...
bool filterData(LidarData d) {  //return false if point is tossed, return false if point is accepted
  ...
  if (d.quality < 15) return false;
  if (d.distance < 150) return false;
  if (d.distance > 6000) return false;

  pointMemory[(uint8_t)(d.angle * 10)] = d.distance;
  return true;
}

void linearEstimate(uint8_t a, uint8_t b) {  //angle a * 10, angle b * 10
